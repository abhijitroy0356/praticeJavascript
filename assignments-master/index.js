...
app.get('/check',(req,res)=>{
    ...
    let ans =totaltime/counter;
    res.json({
        time:ans
    })
})
app.post('/deafult-errorware',(req, res)=>{
    const ans = req.headers.username
    const pass=req.headers.password
    if(ans==='Abhijit' && pass==='1234'){
        next();
    }
    else{
        res.status(401).send("unknow")
    }
},(req,res)=>{
    res.send("first auth done")
})
